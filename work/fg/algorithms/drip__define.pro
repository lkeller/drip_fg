...
;                 Modified DRIP to include GRISM
;     Modified    Luke Keller, Ithaca College, May 2012
;                 Updated to be consistent with FORCAST-DCS Interface Control
;                 Document (SE03-2001)
...
pro drip::setdata, mode=mo, n=n, gmode=gmode, coaddN=coaddN, lastlistN= lastlistN, readme=rm, $
        ...
        extracted=exd, allwave=alw, allflux=alf, $ ; grism spectral extraction
...
if keyword_set(exd) then *self.extracted=exd ; extraced grism spectrum
if keyword_set(alw) then *self.allwave=alw   ; wavelength
if keyword_set(alf) then *welf.allflux=alf   ; "flux" (pre flux cal, really it's response)
...
;if keyword_set(cor) then *self.coadded_rot=cor  ; removed for grism extraction
...
function drip::getdata, mode=mo, gmode=gmode, n=n, lastlistN=lastlistN, coaddN=coaddN, readme=rm, $
             pathcal=wo, pathload=pl, pathsave=ps, pathcalsave=pc, filename=fn, header=hd, lastheader=lhd, basehead=bh, $
             ...
             extracted=exd, allwave=alw, allflux=alf, $ ; GRISM
             merged=md, coadded=coa, coadded_rot=cor, lastcoadd=lc, lastlistcoadd=llc, mergestack=ms, $ ; MERGESTACK
	     lastmergestack=lms, lastditherp=ldp, lastlistmergestack=llms, lastlistditherp=lldp, $
             lastcoaddN=lcn, lastlistcoaddN=llcn,  $
...
if keyword_set(exd) then return, *self.extracted ;GRISM
if keyword_set(alw) then return, *self.allwave   ;GRISM
if keyword_set(alf) then return, *self.allflux   ;GRISM
...
structure={mode:self.mode, gmode:self.gmode, n:self.n, coaddN:self.coaddN, lastlistN:self.lastlistN, readme:self.readme, $ 
           ...
           extracted:*self.extracted, allwave:*self.allwave, allflux:*self.allflux, $ ; GRISM
           ditherp:*self.ditherp, coadded:*self.coadded, $  ;coadded_rot:rot(*self.coadded,90), $ REMOVED for GRISM
...
if keyword_set(mf) then begin
    ...
    data=self.coadded
    head=self.basehead
;endif else if keyword_set(cor) then begin      ; removed for GRISM
...
if keyword_set(all) then begin
	...
	*data = datacube
    ...
    fname=strmid(self.filename,0,namepos)+'_merged.fits' ;GRISM: This is the fully processed spectral image
    data=self.merged
endif else if keyword_set(co) then begin
    fname=strmid(self.filename,0,namepos)+'_coadded.fits'  ; GRISM: This is an extracted spectrum
    data=self.coadded
;endif else if keyword_set(cor) then begin    ; removed for GRISM
;    fname=strmid(self.filename,0,namepos)+'_coadded_rot.fits'
;    data=self.coadded_rot
endif else begin
    fname=strmid(self.filename,0,namepos)+'_reduced_ext.fits' ; GRISM Default is: save extracted, coadded spectra
    data=self.coadded
    ...
    *data=[transpose((*self.coadded)[*,0]),$ ;allwave
          transpose((*self.coadded)[*,1]),$ ;allflux
          transpose((*self.coadded)[*,2]),$
          transpose((*self.coadded)[*,3])]
    collabels=['wavelength', 'flux', 'flux_error','order']
    
    ;self.mw->print,'Written file '+fname  
    ;drip_message, 'Wrote Extracted Spectrum to File'+fname
...
; save file
writefits,fname,*data,*self.basehead ; was fits_write

drip_message,'Saved reduced image data file: '+fname
; free memory space for data pointer
;if ptr_valid(data) then ptr_free,data

;SAVE EXTRACTED GRISM SPECTRA AS FITS TABLE
...
;Find gmode, GRISM mode selection using FILT* keywords
;
...
if(drip_getpar(header,'SPECTRAL') eq 'FOR_XG111' OR $
   drip_getpar(*self.basehead, 'SPECTEL1') eq 'FOR_XG111') then begin
    value='G3xG4'
    self.gmode=1
endif

if(drip_getpar(header,'SPECTRAL') eq 'FOR_G063' OR $
   drip_getpar(*self.basehead, 'SPECTEL1') eq 'FOR_G063') then begin
    value='G1'
    self.gmode=2
endif
if(drip_getpar(header,'SPECTRAL') eq 'FOR_G111' OR $
   drip_getpar(*self.basehead, 'SPECTEL1') eq 'FOR_G111') then begin
    value='G3'
    self.gmode=3
endif    

if(drip_getpar(header,'SPECTRAL') eq 'FOR_G227' OR $
   drip_getpar(*self.basehead, 'SPECTEL1') eq 'FOR_G227') then begin
    value='G5'
    self.gmode=4
endif    
   
if(drip_getpar(header,'SPECTRAL') eq 'FOR_G329' OR $
...
if (self.gmode le 3) then begin
   badpix = self.pathcal+'swc_badpix.fits'  ; was  '../Cal/swc_badpix.fits' 
   dark = self.pathcal+'swc_dark_10262012.fits'
endif
if (self.gmode gt 3) then begin
   badpix = self.pathcal+'lwc_badpix.fits'
...
;***********   END OF GRISM MODE AND FLAT ALLOCATION  ***********
...
;FLATFIELD CORRECTION USED FOR DEFRINGING IN GRISM MODE
;
; read flat fields
;print,'SELF.FLATFILE',self.flatfile

if self.flatfile ne '' then no_flat_data=0  ; Flat data filename found

if self.flatfile eq '' then begin
    self.flatfile=drip_getpar(*self.basehead,'FLATFILE'+camera,writename='FLATFILE')  
    ;print,'FLATFILE ',self.flatfile
    no_flat_data=0 ; 1 = 'No flat data found', 0 = 'flat data found'
endif
...
; Old code to choose grism flatfield mask commented out here
...
; Choose flatfield mask to match GRISM mode format
if self.gmode eq  0 then begin  ;get the order mask
    fname=self.pathcal+'gxd5-8_order_mask.fits' ; was '../Cal/G1xG2_order_mask.fits'
    ordermask=readfits(fname)    ; was readfits(self.pathload+fname)
endif
if self.gmode eq   1 then begin ;get the order mask
    fname=self.pathcal+'G3xG4_order_mask.fits' ; was '../Cal/G3xG4_order_mask.fits'
    ordermask=readfits(fname)
endif
...
if s[0] eq 3 then begin
    CASE 1 of 
       ...
       ;(s[3] eq 2): begin
          ...
          ;if ((self.gmode eq 0) OR (self.gmode eq 1)) then master[where(ordermask eq 0)]=1.
          ;end
       (s[3] lt 4) OR (s[3] gt 4): begin  ; Assume planes are images of same source
...
; replace zero values with the median of the surrounding pixels using
...
if no_flat_data eq 0 then begin
    ...
    ; JAILBAR correction of flatfield image
    master = drip_jbclean(master,header,basehead=basehead)
...
;
; NOTE: MASKFLT is set the dripconf.txt
;
...
;print, 'NO INSTMODE specified to pipeline, data reduciton ABORTED'
...
; ptr_free, self.coadded_rot  ; removed for GRISM
...
self.extracted=ptr_new(/allocate_heap) ; GRISM
*self.extracted=-1
...
; self.coadded_rot=ptr_new(/allocate_heap)   ; removed for GRISM
; *self.coadded_rot=-1
...
;	       {name:"COADDED_ROT",pointer:self.coadded_rot}]  removed for GRISM
...
struct={drip, $
	...
	imglinearized:ptr_new(), $  ; data after correction of image linearity (not pixel linearity)
        ...
        extracted:ptr_new(), $      ; extracted data    ;GRISM
        allwave:ptr_new(), $        ; wavelength data   ;
        allflux:ptr_new(), $        ; flux data         ;
        
        merged:ptr_new(), $         ; merged data
        mergestack:ptr_new(), $     ; mergestack (for C2NC2 mode)
        ditherp:ptr_new(), $
        coadded:ptr_new(), $        ; coadded data
;        coadded_rot:ptr_new(),$     ; coadded rotated  ; removed for GRISM
