...
;             CHOPSUB=CHOPSUB, NODSUB=NODSUB, BASEHEAD=BASEHEAD)
;
; INPUTS:
...
function drip_stack, data, header, posdata=pdata, chopsub=chsub, nodsub=ndsub, basehead=basehead
...
if s[0] ne 3 then begin
    ...
    if keyword_set(basehead) then $
      sxaddpar,basehead,'HISTORY','Stack was not applied (Invalid data)'
    return, data
endif
hs=size(header)
if (hs[0] ne 1) or (hs[2] ne 7) then begin
  drip_message, 'drip_stack - invalid header'
    if keyword_set(basehead) then $
      sxaddpar,basehead,'HISTORY','Stack: Invalid image header'
endif
;**** add frames
...
switch mode of
    'C2': 
    'NAS': ;Grism Nod Along Slit, pipeline test mode
    'NOS': ;Grism Nod Off Slit, pipeline test mode
    ...
    'MAP': begin ; MAPping Mode
       ...
       for i=0,s(3)-1,4 do begin
           ...
           nodsub[*,*,i/4]=chopsub[*,*,i/2]+chopsub[*,*,i/2+1]  ;GRISM. (was - for images)
       endfor
       if keyword_set(chsub) then chsub=chopsub
       ; add nod cycles
       if size(nodsub,/n_dimensions) gt 2 then stacked=total(nodsub,3) $
         else stacked=nodsub  ; chopsub[*,*,0]
...
if keyword_set(basehead) then $
  stacked = drip_jbclean(stacked,header,basehead=basehead) $
else stacked = drip_jbclean(stacked,header)
...
if keyword_set(basehead) then $
  sxaddpar,basehead,'HISTORY','Counts converted to millions of e/s using FRMRATE='+strtrim(frmrate,1)+' and EPERADU='+strtrim(frmrate,1)
