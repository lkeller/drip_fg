...
;if((drip_getpar(header,'FILT4_S') eq 'grism6')) then mode = 5
      ...
      'OPTIMAL': begin
                  ...
                  for j = 0, n_elements(c)-1 DO BEGIN
                     ...
                     for k = 0,n_segments-1 do begin
                        ...
                        collapsed = median(total((piece-median(piece)),1, /NAN),3)
...
yvalue=round(slope*(xvalue-self.boxx0))+self.boxy0  ; LDK 
...
; USING 'WAVE' NOW INSTEAD OF 'XVALUE' SINCE WE ARE ADDING WAVELENGTH CALIBRATION HERE

sub_array = fltarr(n_elements(xvalue),dy+1)  ; LDK
