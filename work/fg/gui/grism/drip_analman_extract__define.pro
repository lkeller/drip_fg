...
if(drip_getpar(header,'SPECTRAL') eq 'FOR_XG063' OR $
   drip_getpar(header, 'SPECTEL1') eq 'FOR_XG063') then value='G1xG2';mode=0
   
if(drip_getpar(header,'SPECTRAL') eq 'FOR_XG111' OR $
   drip_getpar(header, 'SPECTEL1') eq 'FOR_XG111') then value='G3xG4';mode=1

if(drip_getpar(header,'SPECTRAL') eq 'FOR_G063' OR $
   drip_getpar(header, 'SPECTEL1') eq 'FOR_G063') then value='G1';mode=2
   
if(drip_getpar(header,'SPECTRAL') eq 'FOR_G111' OR $
   drip_getpar(header, 'SPECTEL1') eq 'FOR_G111') then value='G3';mode=3

if(drip_getpar(header,'SPECTRAL') eq 'FOR_G227' OR $
   drip_getpar(header, 'SPECTEL1') eq 'FOR_G227') then value='G5';mode=4
   
if(drip_getpar(header,'SPECTRAL') eq 'FOR_G329' OR $
   drip_getpar(header, 'SPECTEL1') eq 'FOR_G329') then value='G6';mode=5
    ...
    if keyword_set(*data) then begin
        ...
        self.xplot->draw_multi, orders=orders, /all, extobj=self.extman
        print,'Spectrum plotted from drip_analman_extract'
    endif
endif
