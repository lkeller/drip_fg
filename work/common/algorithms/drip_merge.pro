...
;
; CALLING SEQUENCE:
;     MERGED=DRIP_MERGE(DATA, FLATTED, HEADER, BASEHEAD=BASEHEAD)
;
; INPUTS:
;     DATA - Data to be merged i.e. frame with target images
;     FLATTED - Used for CORFLT but needs to be reviewed
...
function drip_merge, data, flatted, header, basehead=basehead
...
if s[0] ne 2 then begin
    ...
    if keyword_set(basehead) then $
      sxaddpar,basehead,'HISTORY','merge was not applied (Invalid data)'
    return, data
endif
hs=size(header)
if (hs[0] ne 1) or (hs[2] ne 7) then begin
  drip_message, 'drip_merge - invalid header'
  if keyword_set(basehead) then $
    sxaddpar,basehead,'HISTORY','Merge: Invalid image header'
endif
...
if keyword_set(basehead) then begin
  sxaddpar,basehead,'HISTORY','Merge: X,Y plate scales are '+strtrim(x_plate_scale,1)+','+strtrim(x_plate_scale,1)
  sxaddpar,basehead,'HISTORY','Merge: algorithm used for shift is '+cormerge
endif
...
switch mode of
    'C2': begin ; 2 position chop
	  ...
	  ; If we find more than one peak we combine the different peaks
	  ...
	  endelse
        ...
        ; shift and merge
        if (sqrt(chopx^2+chopy^2)) gt maxshift then begin ; was resize*128
            merged=data
	    if keyword_set(basehead) then $
	      sxaddpar,basehead,'HISTORY','Merge was not applied (Chop was greater than '+strtrim(maxshift,1)+')'
        endif else begin
	    merged=(data-shift(data,chopx,chopy))/2  ; mean
	    if keyword_set(basehead) then $
	      sxaddpar,basehead,'HISTORY','Merge: X, Y shifts are '+strtrim(chopx,1)+','+strtrim(chopy,1)
        endelse
      endelse  
        break
    end
    ...
    'MAP': begin ; MAPping Mode
       ...
       chpcoordsys_read=drip_getpar(header,'CHPCOORD') ; Get chop coord system: 0=SIRF, 1=TARF, 2=ERF
       if chpcoordsys_read ne 'x' then chpcoordsys=float(chpcoordsys_read) $
       else chpcoordsys=0.
       nodcoordsys_read=drip_getpar(header,'NODCOORD') ; Get nod coord system: 0=SIRF, 1=TARF, 2=ERF
       if nodcoordsys_read ne 'x' then nodcoordsys=float(nodcoordsys_read) $
       else nodcoordsys=0.
       sky_angle=float(drip_getpar(header,'SKY_ANGL'))*!pi/180.0
       ...
       noddist_read=drip_getpar(header,'NODAMP')
       if noddist_read ne 'x' then noddist=float(noddist_read) $ ; /plate_scale
       else noddist=0.
       
       nod_angle_read=drip_getpar(header,'NODANGLE')
       if nod_angle_read ne 'x' then nod_angle=float(nod_angle_read)*!pi/180.0 $ ; /plate_scale
       else nod_angle=0.
       ...
       nodraas_read=drip_getpar(header,'NODRAAS')
       if nodraas_read ne 'x' then nodraas=float(nodraas_read) $ ; /plate_scale
       else nodraas=0.
       noddecas_read=drip_getpar(header,'NODDECAS')
       if noddecas_read ne 'x' then noddecas=float(noddecas_read) $ ; /plate_scale
       else noddecas=0.
       ...
       ; input data (stacked)
       chopangr_read=drip_getpar(header,'CHPANGLR')
       ;print,chopangr_read
       if chopangr_read ne 'x' then chopangr=float(chopangr_read) $
       else chopangr=0.
       nodangr_read=drip_getpar(header,'NODANGLR')
       ;print,nodangr_read
       if nodangr_read ne 'x' then nodangr=float(nodangr_read) $
       else nodangr=0.
       ...
       if (cormerge eq 'COR') then begin
           ...
           newnodx=nodx
           newnody=nody
           if (sqrt(nodx^2+nody^2)) lt maxshift then begin  ; was resize*128
               ; cross correlate nod 
               xyshift = 25
	  ...
	  NPEAKS = n_elements(findpeaksout[0,*])
	  ; If we find more than one peak we combine the different peaks
	  ; In theory we should find at least three each time but this did not 
	  ...
	  endelse
       ...
       if (cormerge eq 'COR') or (cormerge eq 'CORFLT') or (cormerge eq 'N') then begin
          ...
          ;** merge choped frames
          if (sqrt(chopx^2+chopy^2)) gt maxshift then begin
	    mergechop=data 
	    if keyword_set(basehead) then $
	      sxaddpar,basehead,'HISTORY','Merge of chop positions was not applied (Chop was greater than '+strtrim(maxshift,1)+')'
          endif else begin
	    mergechop=(data-shift(data,chopx,chopy))/2 ; mean
	    if keyword_set(basehead) then $
	      sxaddpar,basehead,'HISTORY','Merge: X, Y chop shifts are '+strtrim(chopx,1)+','+strtrim(chopy,1)
          endelse
	  ;atv22,mergechop
          ...
          if (sqrt(nodx^2+nody^2)) gt maxshift then begin ;was resize*128
             if (nodbeam eq 0) then merged = mergechop $
             else merged = -mergechop
             ;print, 'off-chip BBBBBBB'
	    if keyword_set(basehead) then $
	      sxaddpar,basehead,'HISTORY','Merge of nod positions was not applied (Nod was greater than '+strtrim(maxshift,1)+')'
          endif else begin
          ; If the nod is smaller than 1/2 the array width (128 pixels), then
          ; assume that the nod is on-chip and subtract nods by shifting (NODAMP)
          if (nodbeam eq 0) then begin
          ...
          endelse
	  if keyword_set(basehead) then $
	    sxaddpar,basehead,'HISTORY','Merge: X, Y nod shifts are '+strtrim(nodx,1)+','+strtrim(nody,1)
       endelse
       endif
       break
   end
...
if keyword_set(basehead) then $
  sxaddpar,basehead,'HISTORY','Merge: Image rotation of '+strtrim(rot_angle,2)
drip_message, 'drip_merge - Image rotation of '+strtrim(rot_angle,2)

merged = rot(merged, rot_angle, cubic=-0.5, missing=0)
