...
;     GUI - Main function
;******************************************************************************
pro gui_lwc, xsize=xs, ysize=ys, dataman=dataman_return, _extra=e
...
if (size(findfile(currentdir+path_sep()+'guiconf.txt')))[0] eq 1 then begin
    ...
    conffilename=dialog_pickfile(/must_exist,/read,title='Load LWC GUI Conf File:')
endelse
...
; base widget
top=widget_base(column=2, title='FORCAST LWC Quick Look Data Reduction', $
...
ctrlbase=widget_base(cbase, /column )
; Pipeline Control Base
ctrlbase1=widget_base(ctrlbase, /row )
; draw base
