...
function drip_dispman::getdata, dispinfocus=dispfoc, disp_id=id, dispname=dname

if keyword_set(dispfoc) then return, self.dispinfocus
if keyword_set(id) then begin
  Ndisp = n_elements(*self.disps)
  outlist = String([(*self.disps)[0]->getdata(/disp_id)])
  for i=1,Ndisp-1 do begin
    outlist = [outlist,String((*self.disps)[i]->getdata(/disp_id))]
  endfor
  return, outlist
endif
if keyword_set(dname) then begin
  ...
  k = where(outlist eq dname)
  if k(0) ne -1 then return, (*self.disps)(k(0)) $
  else return,obj_new()
endif
end
