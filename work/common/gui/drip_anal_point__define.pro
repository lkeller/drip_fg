...
;****************************************************************************
PRO drip_anal_point::update, profile=profile
  ...
  if self.focus or self.show then begin
     ...
     if (imgsize[0] gt 0) then begin ; we have data
        ...
        source=total(buffer[apid],/nan)  ;-total(skybuffer[skyid],/nan) * $
        ...
        noise =  stdev(skybuffer[skyid]) ;+ photon_noise
        
        noise_electrons=noise ;sqrt(n_elements(apid) * (stdev(skybuffer[skyid] * eperadu))^2 $
                   ;+ photon_noise)
                   
        ;s2n=source_electrons/noise_electrons ; calculate S/N ratio
        s2n=source/noise   ; S/N ration in counts (e- conversion now done in drip_stack)
...
;****************************************************************************
;     PLOTPROFILE - plot profile in the aperture
;****************************************************************************

PRO drip_anal_point::plotprofile
  ...
  ; define plot size around the center
  plotsize = fix(self.profzoom*fwhm)
     
  ; get image of all inside outer ring
  x0=self.centu-plotsize > 0
  x1=self.centu+plotsize < imgsize[1] -1
  y0=self.centv-plotsize > 0
  y1=self.centv+plotsize < imgsize[2] -1
  ...
  ; Center coordinate of the buffer
  xc = plotsize
  yc = plotsize
  ; boxsize = mean of inscribed and outer square of aperture circle
  boxsize=fix(0.85*float(self.apradw)+0.5)
  ...
  xcent = ((xc-boxsize)>0) +ans[4] ;self.fitu - (self.centu-plotsize)
  ycent = ((yc-boxsize)>0) +ans[5] ;self.fitv - (self.centv-plotsize)
  
  wset,self.plotwid
  absice = indgen(2*plotsize+1)-plotsize
  fluxmin = min([min(buffer(*,ycent)),min(buffer(xcent,*))])
  fluxmax = max([max(buffer(*,ycent)),max(buffer(xcent,*))])
  !X.MARGIN = [3,0]
  !Y.MARGIN = [2,0]

  plot, absice, buffer[*,ycent],yrange=[fluxmin,fluxmax]
  oplot, absice, buffer[xcent,*], linestyle=2

  ; Rigth side apertures
  oplot,[self.apradw,self.apradw],[fluxmin,(fluxmax+fluxmin)/2],THICK=3.0
  oplot,[self.isradw,self.isradw],[fluxmin,(fluxmax+fluxmin)/4],THICK=2.0,linestyle=2
  ;oplot,[self.isradw,self.osradw],[(fluxmax+fluxmin)/4,(fluxmax+fluxmin)/4],THICK=2.0
  oplot,[self.osradw,self.osradw],[fluxmin,(fluxmax+fluxmin)/4],THICK=2.0,linestyle=2
  ; Left side apertures
  oplot,[-self.apradw,-self.apradw],[fluxmin,(fluxmax+fluxmin)/2],THICK=3.0
  oplot,[-self.isradw,-self.isradw],[fluxmin,(fluxmax+fluxmin)/4],THICK=2.0,linestyle=2
  ;oplot,[-self.isradw,-self.osradw],[(fluxmax+fluxmin)/4,(fluxmax+fluxmin)/4]
  oplot,[-self.osradw,-self.osradw],[fluxmin,(fluxmax+fluxmin)/4],THICK=2.0,linestyle=2
  
  xlevel = (fluxmax+max([0,fluxmin]))*0.7
  ylevel = (fluxmax+max([0,fluxmin]))*0.6
  barsize = 3*(max([self.profzoom/4,1]))
  centerbar = absice(0)
  oplot,[centerbar-barsize,centerbar+barsize],[xlevel,xlevel]
  xyouts,centerbar+1.5*barsize,xlevel,'Along X'
  oplot,[centerbar-barsize,centerbar+barsize],[ylevel,ylevel], linestyle=2
  xyouts,centerbar+1.5*barsize,ylevel,'Along Y'

  ;plot, absice, fitimg(*,ans[5]),yrange=[fluxmin,fluxmax]
  ;oplot, absice, fitimg(ans[4],*), linestyle=2
  ;oplot, absice, gaussian(absice,[ans[1],ans[4]+absice(0),ans[2],ans[0]]), linestyle=4
  ;oplot, absice, gaussian(absice,[ans[1],ans[5]+absice(0),ans[3],ans[0]]), linestyle=3
END
...
; draw circles
; aperture
tvcircle, self.apradz, self.centx, self.centy, color=color, /device
; sky circles
if self.skymode eq 'cent' then begin
    ; inner sky
   tvcircle, self.isradz, self.centx, self.centy, color=color1, /device
   ; outer sky
   tvcircle, self.osradz, self.centx, self.centy, color=color2, /device
endif else begin
   ; outer sky
   tvcircle, self.osradz, self.skyx, self.skyy, color=color2, /device
endelse
...
; profilewid
widget_control, self.profilewid, $
  set_uvalue={object:self, method:'input'}
widget_control, self.profilewid, set_value='Plot'
; datawid
for i=1,4 do begin
...
;****************************************************************************
;     SETPLOTWID - to set widget of plot
;****************************************************************************

pro drip_anal_point::setplotwid, wid, zoomint
  self.plotwid = wid
  self.profzoom = zoomint
end
...
struct={drip_anal_point, $
        ...
        profilewid:0L, $        ; widget id for profile selector 
	profzoom: 0,       $    ; Zomm of profile plot in number of fwhm
        datawid:lonarr(5), $    ; widget ids for data displays (row, wid1, . .
        plotwid:0L, $           ; window id - for graphics
