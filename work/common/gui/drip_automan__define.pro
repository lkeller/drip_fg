...
if self.on then begin
    ...
    while not eof(lun) do begin
		...
		;** if not
		    ...
		    self.pipeman->setpipepanel,'none'
                    ; save old pipe
...
case event.id of
    ...
    (*self.autoconfstat).menu:begin
        if (*self.autoconfstat).stat eq 0 then begin
            ...
            (*self.autoconfstat).done=widget_button(row, value='Done', $
              ...
              uvalue={object:self, method:'autoconf'} )
            (*self.autoconfstat).cancel=widget_button(row, value='Cancel', $
              event_pro='drip_eventhand', $
              uvalue={object:self, method:'autoconf'} )
            ; set status
    ...
    (*self.autoconfstat).cancel:begin
        if (*self.autoconfstat).stat ne 0 then begin
            widget_control, event.top, /destroy
            (*self.autoconfstat).stat=0
        endif
...
if self.on then begin
    pipestat = self.pipeman->checksteatepanel()
    docontinue = 'Yes'
    
    ; Case where no image has been reduced but some images are opened
    if pipestat eq 1 then begin
      dlgmsg = strarr(7)
      openf = (self.pipeman).n
      
      dlgmsg(0) = 'You opened '+strtrim(openf,1)
      if openf eq 1 then $
	dlgmsg(0) = dlgmsg(0)+' file but you did not reduce it. ' $
      else $
	dlgmsg(0) = dlgmsg(0)+' files but you did not reduce them. '
      dlgmsg(1) = 'Switching to Auto-Reduce mode will reset the pipeline'
      if openf eq 1 then $
	dlgmsg(2) = 'and this file will not be processed. ' $
      else $
	dlgmsg(2) = 'and these files will not be processed. '
      dlgmsg(3) = 'If you want to finish processing Cancel the Auto-Reduce switch '
      dlgmsg(4) = 'and press reduce in the main window. '
      dlgmsg(5) = ''
      dlgmsg(6) = 'Do you want to switch to Auto-Reduce mode'
      docontinue = DIALOG_MESSAGE( dlgmsg, /DEFAULT_NO , /CENTER, /QUESTION, TITLE='AUTO-REDUCING?' ) 
      
      ; if Yes reset the current runing pipeline and create a new one
      self.pipeman->new
    endif 
    
    ; Case where images has been reduced and also some others are opened but not reduced
    if pipestat eq 2 then begin
      dlgmsg = strarr(8)
      openf = (self.pipeman).n - (self.pipeman).ind
      dlgmsg(0) = 'You already reduced '+strtrim((self.pipeman).ind,1)+' file
      if (self.pipeman).ind gt 1 then dlgmsg(0) = dlgmsg(0) + 's'
      dlgmsg(0) = dlgmsg(0) + ' and you opened '+strtrim(openf,1)+' more file'
      if openf gt 1 then dlgmsg(0) = dlgmsg(0) + 's'
      dlgmsg(1) = 'that you did not reduce yet. '
      dlgmsg(2) = 'Switching to Auto-Reduce mode will reset the pipeline '
      if openf eq 1 then $
	dlgmsg(3) = 'and this file will not be processed. ' $
      else $
	dlgmsg(3) = 'and these files will not be processed. '
      dlgmsg(4) = 'If you want to finish processing Cancel the Auto-Reduce switch '
      dlgmsg(5) = 'and press reduce in the main window. '
      dlgmsg(6) = ''
      dlgmsg(7) = 'Do you want to switch to Auto-Reduce mode'
      docontinue = DIALOG_MESSAGE( dlgmsg, /DEFAULT_NO , /CENTER, /QUESTION, TITLE='AUTO-REDUCING?' ) 
    ...
    if docontinue eq 'Yes' then begin      
      ; set widgets
    ...
    widget_control, self.automenu, set_value='Start Auto Reduce'
    ; check what is the status of the pipeline
    ; because we can't do step back in auto-reduce we assume there 
    ; was not a step back. There is a case where this could failed.
    ; If the auto-reduce has been called just after a step back but there
    ; is not reduced files after auto-reduce is called.
    ; I leave it like this for now because this is very unlikely 
    ; and it is too much implementation
    ; For example, we could do:
    ; if (*((*self.pipeman).pipestat)).didstepback eq 1 then
    ;     and no file has been reduced in the mean time then no change for didstepback (this is the tricky condition)
    ; else then change as below
    (*((self.pipeman).pipestat)).didstepback = 0
    self.pipeman->setpipepanel,'state'
endelse
...
*self.autoconfstat={stat:0, listfileid:0L, listfilebutton:0L, $
                    ...
                    menu:autoconf, done:0L, cancel:0L}
