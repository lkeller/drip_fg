...
if self.focus then begin
   ...
   endelse
    ; update atv image
    ; we could add something that checks if the atv is opened
    ; and update only if it is 
    ;atv22,*(self.dataraw)
endif
end
...
; pass image to atv if current image selected
end
...
; check if old dap selection available else set to first dap
dapi=-1
repeat dapi=dapi+1 $
