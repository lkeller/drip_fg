...
;   starting with PARNAME. A lowercase search is performed if the
;   keyword is not found. It then searches for the first non-space
...
;   Modified: Marc Berthoud, Yerkes Observatory July 2010
;             - Added search for lowercase version of the keyword
;             - Added ignoring keyword if line is commented out
...
;
  ...
  ; search for name (uppercase)
  nfound=where((strpos(list,name) ge 0) and $
               (strpos(list,name) lt strpos(list,'=')) and $
               (strpos(list,';') ne 0), matches)
  if matches lt 1 then begin
     ; search for name (lowercase)
